...
library Crypto {
    ...
    function isPrime(BigNumber memory a, BigNumber[3] memory randomness) internal view returns (bool) {
        ...
        int256 j;
        uint256 num_checks = primeChecksForSize(a.bitlen);
        ...
        returns (uint256)
    {
        ...
        /// decipher
        uint256 decipherlen = _m.val.length;
    ...
    function getK(BigNumber memory a1) private pure returns (uint256 k) {
        k = 0;
        uint256 mask = 1;
        uint256 a1_ptr;
        uint256 val;
    ...
    function primeChecksForSize(uint256 bit_size) private pure returns (uint256 checks) {
        checks = bit_size >= 1300
            ? 2
            : bit_size >= 850
                ? 3
                : bit_size >= 650
    ...
    modifier verifyParams(BigNumber memory s, BigNumber memory e, BigNumber memory m) {
        s.verify();
        e.verify();
        m.verify();
        _;
