...
contract TexasHoldemRoom {
    ...
    uint256 public constant EMPTY_SEAT = 255;
    ...
    uint256[MAX_PLAYERS] public seatPositionToPlayerIndex;
    ...
    event PotWon(address[] winners, uint256[] winnerPlayerIndexes, uint256 amount);
    event InvalidCardsReported(address indexed player);
    event PlayerJoined(
        address indexed player, uint256 indexed playerIndex, uint256 indexed seatPosition
    );
    event PlayerLeft(
        address indexed player, uint256 indexed playerIndex, uint256 indexed seatPosition
    );
    ...
    function getNextActivePlayer(bool requireActive) public view returns (uint256) {
        ...
        uint256 currentSeatPosition = players[currentPlayerIndex].seatPosition; // 1, 1
        ...
        uint256 nextSeatIndex = (currentSeatPosition + 1) % MAX_PLAYERS; // 2 % 2 = 0
        while (nextSeatIndex != currentSeatPosition) {
            // TODO: add a status for a player that has joined, but did not start the round
            uint256 playerIndex = seatPositionToPlayerIndex[nextSeatIndex];
    ...
    function _startNewHand() internal {
        ...
        uint256 previousDealerSeatPosition = players[dealerPosition].seatPosition;
        ...
        uint256 nextDealerSeatPosition = (previousDealerSeatPosition + 1) % MAX_PLAYERS;
        while (
    ...
    function determineWinners() internal {
        ...
        uint256 maxWinnerCount = countOfHandsRevealed() > 0 ? countOfHandsRevealed() : 1;
        uint256[] memory winnerPlayerIndexes = new uint256[](maxWinnerCount);
        uint256 winnerCount = 0;

        if (countActivePlayers() == 1) {
            // emit THP_Log("_progressGame() dw in_countActivePlayers() == 1");
            // only 1 active player, so they win the pot
            uint256 lastActivePlayerIndex;
            for (uint256 i = 0; i < MAX_PLAYERS; i++) {
                if (
        ...
        uint256[] memory justWinnerIndicies = new uint256[](winnerCount);
        for (uint256 i = 0; i < winnerCount; i++) {
            uint256 winnerPlayerIndex = winnerPlayerIndexes[i];
    ...
    function reportIdlePlayer() external {
        ...
        uint256 playerIndexToKick = EMPTY_SEAT;
    ...
    function countActivePlayers() public view returns (uint256) {
        uint256 count = 0;
        for (uint256 i = 0; i < MAX_PLAYERS; i++) {
            if (
    ...
    function setPlayerHandScore(uint256 playerIndex, uint256 handScore) external {
        require(msg.sender == address(deckHandler), "Only DeckHandler can call this");
    ...
    function getPlayer(uint256 playerIndex) external view returns (Player memory) {
        return players[playerIndex];
    }

    function countPlayersAtRoundStart() external view returns (uint256) {
        uint256 count = 0;
        for (uint256 i = 0; i < MAX_PLAYERS; i++) {
    ...
    function getPlayersCardIndexes(uint256 playerIndex)
        external
        view
        returns (uint256[2] memory playerCardIndexes)
    {
        // emit THP_Log("_progressGame() in getPlayersCardIndexes()");
        uint256 countOfPlayersCounterClockwiseToDealer = 0;
        uint256 playerSeatPosition = players[playerIndex].seatPosition; // 0
        uint256 dealerSeatPosition = players[dealerPosition].seatPosition; // 1
        ...
        uint256 playersAtRoundStart = this.countPlayersAtRoundStart();
     ...
     */
    function countOfHandsRevealed() public view returns (uint256) {
        uint256 count = 0;
        for (uint256 i = 0; i < MAX_PLAYERS; i++) {
